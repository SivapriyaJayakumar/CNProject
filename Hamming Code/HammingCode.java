    ...
    public static void main(String args[]){
        ...
        for(int i=1;i<=data_redundancy.length;i++){
            if(i==1 || obj.checkPowerOfTwo(i)){
                ...
                System.out.println("i is  "+i+"  Parity received is "+ received_parity[k_redind]);
                k_redind+=1;
            }
            else{
                received_pure_data[dataind]=data_received[i-1];
                System.out.println("i is  "+i+"Pure Data received is "+   received_pure_data[dataind]);
                dataind+=1;
            }
        }
        received_parity_check=new int[redundancybits.length];
        received_parity_check=obj.calculateParityBits(redundancybits.length,received_pure_data);
